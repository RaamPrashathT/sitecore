...
export async function sendPartialOrderNotification(
    ...
    try {
        ...
        const itemsHtml = items
            ...
            <tr style="border-bottom: 1px solid #eee;">
                ...
                <td style="padding: 12px; text-align: center; font-size: 14px; color: #333;">
                    ${item.supplierQuantity} ${item.unit}
                </td>
                <td style="padding: 12px; text-align: center; font-size: 14px; color: #999;">
                    ${item.inventoryQuantity} ${item.unit} (from stock)
                </td>
                <td style="padding: 12px; text-align: right; font-size: 14px; color: #333;">
                    ₹${(item.truePrice * item.supplierQuantity).toFixed(2)}
                </td>
            </tr>
        `,
            )
        ...
        const totalSupplierAmount = items.reduce(
            (sum, item) => sum + item.truePrice * item.supplierQuantity,
            0,
        );
        ...
        const { data, error } = await resend.emails.send({
            ...
            subject: `Partial Order from ${organizationName}`,
            html: `
                <div style="font-family: sans-serif; max-width: 640px; margin: auto; border: 1px solid #ddd; padding: 24px; border-radius: 10px; background-color: #f9f9f9;">
                    ...
                    <p style="font-size: 14px; color: #444; line-height: 1.6; margin-bottom: 16px;">
                        We would like to order the following materials to be sent to our project location. Note that some items will be fulfilled from our existing inventory, and only the quantities below need to be supplied by you.
                    </p>
                        ...
                        <table style="width: 100%; border-collapse: collapse;">
                            <thead>
                                <tr style="background-color: #15803d; color: white;">
                                    ...
                                    <th style="padding: 12px; text-align: center; font-weight: 600; font-size: 14px; color: #d1d5db;">
                                        From Stock
                                    </th>
                    ...
                    <div style="background-color: #f0f0f0; padding: 16px; border-radius: 6px; margin-bottom: 24px; text-align: right;">
                        <p style="font-size: 14px; color: #666; margin: 0 0 8px 0;">
                            <strong>Total Amount (Your Supply):</strong> ₹${totalSupplierAmount.toFixed(2)}
                        </p>
                    </div>

                    <p style="font-size: 14px; color: #444; line-height: 1.6; margin-bottom: 24px;">
                        Please confirm the availability and expected delivery date for the quantities mentioned above. If you have any questions or need clarification on this order, please don't hesitate to contact us.
                    </p>
        ...
        logger.info(
            `Partial order notification sent to supplier ${supplierName} (${supplierEmail})`,
        );
        return { success: true, data };
    } catch (err) {
        logger.error("Failed to send partial order notification:", err);
